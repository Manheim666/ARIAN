...
# ============================================================================
# 8. Temperature range and degree-days
# ============================================================================

def add_temperature_range(
    df: pd.DataFrame,
    temp_max_col: str = "temperature_2m_max",
    temp_min_col: str = "temperature_2m_min",
) -> pd.DataFrame:
    """Add daily temperature range (max - min) as a volatility indicator."""
    if temp_max_col not in df.columns or temp_min_col not in df.columns:
        logger.warning("Temperature range: %s or %s missing", temp_max_col, temp_min_col)
        return df
    
    df = df.copy()
    df["temperature_range"] = df[temp_max_col] - df[temp_min_col]
    logger.info("Added temperature_range feature")
    return df


def add_degree_days(
    df: pd.DataFrame,
    temp_mean_col: str = "temperature_2m_mean",
    base_temp: float = 18.0,
) -> pd.DataFrame:
    """Add heating and cooling degree days as energy demand proxies.
    
    HDD = max(0, base_temp - T_mean)  # heating demand
    CDD = max(0, T_mean - base_temp)  # cooling demand
    """
    if temp_mean_col not in df.columns:
        logger.warning("Degree days: %s missing", temp_mean_col)
        return df
    
    df = df.copy()
    temp = df[temp_mean_col].fillna(0)
    df["heating_degree_days"] = np.maximum(0, base_temp - temp)
    df["cooling_degree_days"] = np.maximum(0, temp - base_temp)
    logger.info("Added heating_degree_days and cooling_degree_days features (base=%.1f°C)", base_temp)
    return df
...
def build_weather_features(
    ...
    # --- 2. Temperature range and degree days ---
    df = add_temperature_range(df)
    df = add_degree_days(df)

    # Extend context columns to include the newly-created wind components and temp features
    ctx = list(cfg.context_columns)
    for extra in ("wind_u", "wind_v", "temperature_range", "heating_degree_days", "cooling_degree_days"):
